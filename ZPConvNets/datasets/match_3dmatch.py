...
class FragmentTestLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # if self.use_normals:
        #     for pca,pcb,na,nb in zip(rawA, rawB, normalA, normalB):
        #         inputA.append(self._preprocess(pca,na))
        #         inputB.append(self._preprocess(pcb,nb))
        # else:
        for pca, pcb in zip(rawA, rawB):
...
class SceneEvalLoader(data.Dataset):
    def __init__(self, opt, scene):
        ...
        #                 o3d.io.read_point_cloud(os.path.join(self.data_path, \
    ...
    def __getitem__(self, index):
        ...
        ########################################
        # if we want downsampling...
        # frag = frag.voxel_down_sample(voxel_size=0.015) #0.015
        ##############################################
...
# This dataloader provides batches of point clouds extracted around key points
class ScenePairDataLoader(data.Dataset):
    def __init__(self, opt, shuffle=True):
        super(ScenePairDataLoader, self).__init__()
        self.opt = opt
        ...
        # self.target_folder = opt.training_data_folder if mode == 'train' else opt.evaluate_input_folder
        target_folder = self.opt.dataset_path
        self.files = glob.glob(os.path.join(target_folder, 'all_patches', "*.mat"))
        self.pose_table = sio.loadmat(os.path.join(target_folder, "pose_table.mat"))

        print("[Dataloader] : Training dataset size: %d" %(len(self.files) * self.scene_size))
        self.scene = None

    def __len__(self):
        return len(self.files) * self.scene_size
    ...
    def __getitem__(self, index):
        ...
        return data
    def _load_instance(self, sid, pid):
        '''
            src, tgt: [(pc, tag)]
        '''

        # sid = min(len(self.files) - 1, sid)
        self.scene = sio.loadmat(self.files[sid])
        # print(sid, pid)

        # pid = min(len(scene['src']) - 1, pid)
        src_pc, src_tag = self.scene['src'][pid]
        tgt_pc, tgt_tag = self.scene['tgt'][pid]

        # Transformation: tgt -> src
        srcR = RigidMatrix(self.pose_table[src_tag[0]])
        tgtR = RigidMatrix(self.pose_table[tgt_tag[0]])

        T = srcR.T * tgtR

        # only takes the rotation part from the rigid transformation
        return src_pc, tgt_pc, T.R
####################DEPRECATED############################################
####################DEPRECATED############################################
####################DEPRECATED############################################
####################DEPRECATED############################################
