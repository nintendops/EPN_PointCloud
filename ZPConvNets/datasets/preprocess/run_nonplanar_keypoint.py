...
def cross_filtering_via_fpfh(points_i, points_j, fpfh_i, fpfh_j, cfg):
    ...
    # filter out nonplanar point, aka std() >= 20
    # import ipdb; ipdb.set_trace()
    match_flags = []
    for idx, fs in enumerate(zip(fi,fj)):
        fii, fjj = fs
        if all([fii.std() < 20, fjj.std()<20]):
            match_flags += [idx]
    ####################################################################
...
def run_KeypointSelection(root_path, output_path, cfg):
    ...
    for sdir in get_folders(root_path):
        ...
        ckpt_path = os.path.join(output_path, 'nonplanar', scene_name)
...
if __name__ == '__main__':
    ...
    output_root = r'/home/ICT2000/chenh/Haiwei/Datasets/MScenes/train'
    
    cfg = Config()
    run_KeypointSelection(fuse_root, output_root, cfg)
    print("Done!!!")
