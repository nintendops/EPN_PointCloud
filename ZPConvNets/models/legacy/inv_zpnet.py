...
class InvZPConvModel(nn.Module):
    def __init__(self, params):
        super(InvZPConvModel, self).__init__()

        self.backbone = nn.ModuleList()
        for block_param in params['backbone']:
            self.backbone.append(M.BasicZPConvBlock(block_param))
        self.outblock = M.InvOutBlock(params['outblock'])
...
def build_model(device,
                ...
                anchors=[42,42,42],# [ANC]*4, [92, 42, 42, 12]
                aperture_ratio = [0.25, 0.5, 1.0,],
                sampling_ratio = 2,
                kernel_density = 1.5,
                kernel_multiplier = 2,
                input_radius=0.15 * 3**0.5,
...
#                 to_file=None):
    ...
    for i, block in enumerate(mlps):
        ...
        for j, dim_out in enumerate(block):
            ...
            # stride at first
            inter_stride = 1
            inter_sigma = sigma_ratio * radius[i] * stride_multipliers[i+1]**0.5 / radius[0]

            if j == 0:
                inter_stride = strides[i]
                inter_sigma = sigma_ratio * radius[i] * stride_multipliers[i]**0.5 / radius[0]
    ...
    model = InvZPConvModel(params).to(device)
    return model
